...
<main class="form-signin w-100 m-auto animate__animated animate__fadeInUp">
  ...
  <form method="POST">
  <?php  $email_mess="";
        if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['pass']) && isset($_POST['date'])){
            ...
            $party_id = bin2hex(random_bytes(5));
            include("db.php");
            $db = new Database;
            $email_db = $db->query("SELECT `email` FROM `admins` WHERE `email`='$email' LIMIT 1");
          
            if(empty($email_db)){
                 $db->execute("INSERT INTO `admins` SET `name`='$name', `email`='$email', `user_token`='$user_token', `password`='$password' , `date`='$date', `party_id`='$party_id'"); 
                 $email_mess="";
          ...
          $email_mess = 'Заполните все поля';
        }
       
    ?>
    <img class="mb-4 animate__animated animate__fadeInUp" src="img/logo.svg" alt="" width="72" height="57">
    <h1 class="h3 mb-3 fw-normal intro animate__animated animate__fadeInUp"><b>GiftMe</b><br><p class="description animate__animated animate__fadeInUp">С наступающим вас! :)<br>Продолжим наше знакомство</p></h1>
    <div class="form-floating animate__animated animate__fadeInUp">
      <input type="name" name="name" id="name" class="form-control animate__animated animate__fadeInUp"  placeholder="Меня зовут">
      <label for="name"><i class="fa-solid fa-user-large animate__animated animate__fadeInUp"></i><span class="label">Меня зовут...</span></label>
    </div>
    <div class="form-floating animate__animated animate__fadeInUp">
    ...
    <div class="form-floating animate__animated animate__fadeInUp">
      <input type="password" name="pass" id="pass" class="form-control animate__animated animate__fadeInUp"  placeholder="Меня зовут">
      <label for="pass"><i class="fa-solid fa-key animate__animated animate__fadeInUp"></i><span class="label">Пароль</span></label>
    </div>
    <div class="form-floating animate__animated animate__fadeInUp">
      <input type="date" name="date" id="date" class="form-control animate__animated animate__fadeInUp"  placeholder="Меня зовут">
      <label for="date"><span class="label">Дата рождения</span></label>
    </div>
